...
- **Auth (REST)**: verifies the Clerk JWT with the official `clerk-backend-api` SDK (`authenticate_request`, networkless via JWKS). Actions are attributed to the Miembro.
- **Auth (MCP)**: `Authorization: Bearer <token>` per Miembro (ADR-0001); an ASGI gate resolves the token → Miembro → Familia via the `resolve_mcp_token` SECURITY DEFINER function (the only deliberate RLS bypass, for the auth bootstrap) and returns **401** on invalid/revoked tokens before any tool runs. Each tool opens its own transaction and sets `app.current_family_id` (SET LOCAL) so RLS applies. Strict Hijo matching (`resolve_child_by_name`) and rate limiting (reverse proxy, ADR-0006) are documented contracts reused by later phases.
